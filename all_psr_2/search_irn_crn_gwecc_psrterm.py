...
psrlist = ["J1713+0747", "J1946+3417", "J2322+2057"]
...
target_params = json.load(open(f"{datadir}/3c66b_params.dat", "r"))
...
if psrlist is not None:
    parfiles = [x for x in parfiles if x.split('/')[-1].split('.')[0].split('_')[0] not in psrlist]
    timfiles = [x for x in timfiles if x.split('/')[-1].split('.')[0].split('_')[0] not in psrlist]
